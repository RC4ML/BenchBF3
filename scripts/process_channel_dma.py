...
target_dir = "/home/cxz/result/channel_dma_new"

bf3_read_target_pattern = r"host-bf3_p(\d+)_b(\d+)_t(\d+)_read_export"
bf3_write_target_pattern = r"host-bf3_p(\d+)_b(\d+)_t(\d+)_write_export"

host_read_target_pattern = r"host-bf3_p(\d+)_b(\d+)_t(\d+)_read_export"
host_write_target_pattern = r"host-bf3_p(\d+)_b(\d+)_t(\d+)_write_export"
    ...
    for fi in os.listdir(target_dir):
        if re.match(pattern, fi):
            ...
            if thread_num != num_thread:
                continue
            if batch_size not in now_inflight_map:
                continue
