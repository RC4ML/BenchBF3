...
void post_dma_reqs(uint64_t thread_id, DOCA::bench_runner *runner, task_config *task_config) {
    ...
    uint32_t batch_size = task_config->gdr_config->batch_size;
    size_t mod_num = task_config->gdr_config->mmap_length - task_config->gdr_config->payload;
    (void)mod_num;
    ...
    auto complete_callback = [](struct doca_dma_task_memcpy *useless_task, union doca_data task_user_data,
        union doca_data ctx_user_data) -> void {
            ...
            delete task_config->task_src_buf[task_index];
            delete task_config->task_dst_buf[task_index];
            delete task_config->task_buffers[task_index];

            task_config->task_buffers[task_index] = nullptr;
            task_config->finish_index++;
        };
    ...
    while (runner->running()) {
        while (task_config->now_index - task_config->finish_index < batch_size) {
            size_t index = task_config->now_index % batch_size;
            if (task_config->task_buffers[index] != nullptr) {
                break;
            }
            task_config->now_index++;

            // size_t remote_offset = distrib(engine);
            // remote_offset = DOCA::round_up(remote_offset, CACHE_LINE_SZ);
            // remote_offset = remote_offset % mod_num;
            size_t remote_offset = index * task_config->gdr_config->payload;
            size_t local_offset = index * task_config->gdr_config->payload;

            if (task_config->gdr_config->is_read) {
                task_config->task_src_buf[index] = DOCA::buf::buf_inventory_buf_by_data(task_config->inv, task_config->remote_mmap, task_config->remote_mmap_addr + remote_offset, task_config->gdr_config->payload);
                task_config->task_dst_buf[index] = DOCA::buf::buf_inventory_buf_by_addr(task_config->inv, task_config->local_mmap, task_config->local_mmap_addr + local_offset, task_config->gdr_config->payload);
            } else {
                task_config->task_src_buf[index] = DOCA::buf::buf_inventory_buf_by_data(task_config->inv, task_config->local_mmap, task_config->local_mmap_addr + local_offset, task_config->gdr_config->payload);
                task_config->task_dst_buf[index] = DOCA::buf::buf_inventory_buf_by_addr(task_config->inv, task_config->remote_mmap, task_config->remote_mmap_addr + remote_offset, task_config->gdr_config->payload);
            }
    ...
    std::cout
        << DOCA::string_format("latency %.4f\n",
            double(task_config->stat->timer.get_now_timepoint()) / (1000 * task_config->stat->num_ops_finished));
}
