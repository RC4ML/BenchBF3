...
var (
	...
	codec      = flag.String("codec", "cauchy", "Encoder Algorithm")
	...
	duration   = flag.Int("duration", 5, "Minimum number of seconds to run.")
	progress   = flag.Bool("progress", false, "Display progress while running")
	concurrent = flag.Bool("concurrent", false, "Run blocks in parallel")
	cpu        = flag.Int("cpu", 1, "Set maximum number of cores to use")
...
func main() {
	...
	each = (each + padding - 1) / padding * padding
...
func benchmarkEncoding(enc reedsolomon.Encoder, data [][][]byte) {
	...
	for time.Now().Before(end) {
		for _, shards := range data {
			totalLatStart := time.Now()
			err := enc.Encode(shards)
			exitErr(err)
			totalLat += time.Now().Sub(totalLatStart)
			totalCnt++
	...
	fmt.Printf("encode %.2f %.2f\n", float64(totalLat.Microseconds())/float64(totalCnt), speedBitMul*speed)
}
