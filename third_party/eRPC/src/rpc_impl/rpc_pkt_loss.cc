...
namespace erpc
{
  ...
  void Rpc<TTr>::pkt_loss_scan_st()
  {
    ...
    while (cur != &active_rpcs_tail_sentinel_)
    {
      ...
      {
        // ERPC_WARN("Rpc %u, lsn %u, msg %s, req_num %ld, index %ld, credits %ld, cur: Packet loss detected. Retransmitting.\n",
        //           rpc_id_, cur->session_->local_session_num_, cur->progress_str().c_str(), cur->get_cur_req_num(), cur->index_, cur->session_->client_info_.credits_);
