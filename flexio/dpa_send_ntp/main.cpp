#include "dpa_send_ntp.h"

DOCA_LOG_REGISTER(dpa_send_ntp);
...
int main(int argc, char **argv) {
    ...
    config.ctx->create_process(dpa_send_ntp_device);
    config.ctx->create_window();

    config.ctx->generate_flexio_uar();
    config.ctx->print_init("dpa_send", 0);
    config.ctx->create_event_handler(dpa_send_ntp_device_event_handler);
    ...
    Assert(flexio_process_call(config.ctx->get_process(), &dpa_send_ntp_device_init, &rpc_ret_val, dev_config_data) ==
    ...
    Assert(flexio_process_call(config.ctx->get_process(), &dpa_send_ntp_deivce_first_packet, &rpc_ret_val, 0) ==
        FLEXIO_STATUS_SUCCESS);
