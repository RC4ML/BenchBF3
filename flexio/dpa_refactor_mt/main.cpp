...
DOCA_LOG_REGISTER(DPA_REFACTOR_MT);
...
int main(int argc, char **argv) {
    ...
    global_ctx->alloc_pd();
    global_ctx->create_process(dpa_refactor_mt_device);
    global_ctx->create_window();
    global_ctx->generate_flexio_uar();
    global_ctx->print_init("dpa_refactor_mt", 0);
    ...
    std::vector<dpa_refactor_mt_config> configs;
    for (size_t i = 0; i < FLAGS_g_thread_num; i++) {
        dpa_refactor_mt_config config{};
        global_ctx->create_event_handler(dpa_refactor_mt_device_event_handler);
        ...
        Assert(flexio_process_call(global_ctx->get_process(), &dpa_refactor_mt_device_init, &rpc_ret_val,
