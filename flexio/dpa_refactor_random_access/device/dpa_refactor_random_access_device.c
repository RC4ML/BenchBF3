...
flexio_dev_rpc_handler_t dpa_refactor_random_access_device_init;            /* Device initialization function */
flexio_dev_event_handler_t dpa_refactor_random_access_device_event_handler; /* Event handler function */

#define TARGET_PKT_SIZE 1024

uint64_t array_size = LOG2VALUE(LOG_SQ_RING_DEPTH + LOG_WQ_DATA_ENTRY_BSIZE) / sizeof(size_t);
...
process_packet(struct flexio_dev_thread_ctx *dtctx) {
    ...
    if (data_sz == TARGET_PKT_SIZE) {
        uint64_t dummy = 0;
        size_t *random_access_data_ptr = (size_t *)(dev_ctx.dt_ctx.sq_tx_buff);
        size_t *tmp_ptr = (size_t *)(rq_data + 256);
        for (;;tmp_ptr++) {
            size_t tmp = *tmp_ptr;
            if (tmp == MAGIC_NUMBER) {
                break;
            }
            dummy += *(random_access_data_ptr + (tmp % array_size));
        }
        total_dummy += dummy;
...
process_packet_host(struct flexio_dev_thread_ctx *dtctx) {
    ...
    if (data_sz == TARGET_PKT_SIZE) {
        uint64_t dummy = 0;
        size_t *random_access_data_ptr = (size_t *)(dev_ctx.host_rq_ctx.dpa_rx_buff);
        size_t *tmp_ptr = (size_t *)(rq_data_dpa + 256);
        for (;;tmp_ptr++) {
            size_t tmp = *tmp_ptr;
            if (tmp == MAGIC_NUMBER) {
                break;
...
dpa_refactor_random_access_device_init(uint64_t data) {
    ...
    LOG_I("Thread main init success\n");
    return 0;
}
...
void
__dpa_global__
dpa_refactor_random_access_device_event_handler(uint64_t __unused arg0) {
