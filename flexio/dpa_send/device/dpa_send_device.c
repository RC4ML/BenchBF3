...
flexio_dev_rpc_handler_t dpa_send_device_init;            /* Device initialization function */
flexio_dev_rpc_handler_t dpa_send_deivce_first_packet;    /* First packet function */
flexio_dev_event_handler_t dpa_send_device_event_handler; /* Event handler function */
...
dpa_send_device_init(uint64_t data) {
...
__dpa_rpc__ uint64_t dpa_send_deivce_first_packet(uint64_t __unused dummy) {
    ...
    if (dev_ctx.sq_on_host) {
        ...
        uint64_t *data_ptr = (uint64_t *)((char *)sq_data_dpa + hdr_size);
        *(data_ptr + 1) = dev_ctx.send_index;
        *data_ptr = __dpa_thread_cycles();
        finish_send_packet_host(dtctx, &dev_ctx.sq_ctx);
    } else {
...
void
__dpa_global__
dpa_send_device_event_handler(uint64_t __unused arg0) {
    struct flexio_dev_thread_ctx *dtctx;
