#include "dpa_refactor_workingset.h"

DOCA_LOG_REGISTER(DPA_REFACTOR_WORKINGSET);
...
int main(int argc, char **argv) {
    ...
    dpa_refactor_workingset_config config;
    ...
    config.ctx->create_process(dpa_refactor_workingset_device);
    config.ctx->create_window();

    config.ctx->generate_flexio_uar();
    config.ctx->print_init("dpa_refactor_workingset", 0);
    config.ctx->create_event_handler(dpa_refactor_workingset_device_event_handler);
    ...
    Assert(flexio_process_call(config.ctx->get_process(), &dpa_refactor_workingset_device_init, &rpc_ret_val, dev_config_data) ==
